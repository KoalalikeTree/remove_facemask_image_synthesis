...
class ContentLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        self.loss = F.mse_loss(pred, target.detach())
...
class StyleLoss(nn.Module):
    ...
    def forward(self, pred_feature, target_feature):
        # need to cache the appropriate loss value in self.loss
        self.loss = F.mse_loss(gram_matrix(pred_feature), gram_matrix(target_feature).detach())
        return input
